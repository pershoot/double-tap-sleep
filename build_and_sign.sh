...
if [ ! -f "$KEYSTORE" ]; then
    ...
    keytool -genkey -v -keystore "$KEYSTORE" -storepass "$KS_PASS" -alias androiddebugkey -keypass "$KS_PASS" -keyalg RSA -keysize 2048 -validity 10000 -dname "CN=Android Debug,O=Android,C=US"
fi
